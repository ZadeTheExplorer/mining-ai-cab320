...
class Mine(search.Problem):
    ...
    def payoff(self, state):
        ...
        if (self.underground.ndim == 3):
            payoff = sum((x,y,z) for (x,y,z) in self.underground if (x,y) in state == self.underground(x,y,))
        
        # Sum the payoffs in underground, if the coordinate in state matches the coordinate in underground
